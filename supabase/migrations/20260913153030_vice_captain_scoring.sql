...
begin
  if not exists (
    select 1 from parents where id = auth.uid() and is_admin and season_id = (select season_id from matches where id = p_match_id)
  ) then
  ...
  for v_pred in select * from predictions where match_id = p_match_id loop
    ...
    -- Captain bonus, with vice-captain fallback: if the correctly-guessed
    -- MVP is your captain, bonus. Otherwise, if it's your vice-captain,
    -- same bonus -- the badge simply passes down, same as real fantasy football.
    select exists (
