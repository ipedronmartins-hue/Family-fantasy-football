-- fantasy_lineups becomes per-gameweek (per match), not fixed for the whole
-- season -- matching how real Fantasy Premier League works: pick your XI
-- fresh before each match's deadline.
alter table fantasy_lineups add column match_id uuid references matches(id) on delete cascade;

drop index if exists one_captain_per_team;
alter table fantasy_lineups drop constraint if exists fantasy_lineups_fantasy_team_id_player_id_key;

-- Backfill safety: nothing existed yet (checked before migrating), so no
-- data to reconcile. New rows must always specify match_id going forward.
alter table fantasy_lineups alter column match_id set not null;

alter table fantasy_lineups add constraint fantasy_lineups_team_match_player_key
  unique (fantasy_team_id, match_id, player_id);

create unique index one_captain_per_team_per_match
  on fantasy_lineups(fantasy_team_id, match_id) where is_captain;
create unique index one_vice_captain_per_team_per_match
  on fantasy_lineups(fantasy_team_id, match_id) where is_vice_captain;

-- Scoring engine: captain/vice-captain now looked up for THIS match, not
-- the team's all-time (nonexistent) fixed lineup.
...
begin
  ...
  for v_pred in select * from predictions where match_id = p_match_id loop
    ...
    -- Captain/vice-captain now scoped to THIS gameweek's selection.
    select exists (
      select 1 from fantasy_lineups fl
      where fl.fantasy_team_id = v_pred.fantasy_team_id
        and fl.match_id = p_match_id
    ...
    select exists (
      ...
      where fl.fantasy_team_id = v_pred.fantasy_team_id
        and fl.match_id = p_match_id
        and fl.player_id = v_pred.predicted_mvp_id
        and fl.is_vice_captain
    ) into v_is_vice_captain;
    ...
    elsif v_mvp_hit and v_is_vice_captain then
      v_points := v_points + (v_rules->'bonus'->>'captain')::int;
      v_breakdown := v_breakdown || jsonb_build_object('viceCaptainBonus', (v_rules->'bonus'->>'captain')::int);
    end if;
