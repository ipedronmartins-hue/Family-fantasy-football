...
const HOW_IT_WORKS = [
  ...
  { icon: "📈", title: "Sobe no ranking", text: "Compete com os restantes pais ao longo da época." },
];
...
export default async function InicioPage({ params }: { params: Promise<{ teamSlug: string }> }) {
  const { teamSlug } = await params;
  const team = await getTeamBySlug(teamSlug);
  const base = `/${teamSlug}`;

  const [nextMatch, fixtures, parent] = await Promise.all([
    getNextFixture(team.seasonId),
    getFixtures(team.seasonId),
    getCurrentParent(),
  ]);
    ...
    <div className="mx-auto flex w-full max-w-md flex-1 flex-col pb-20">
      ...
      <main className="flex-1 px-5 pt-6">
        ...
        <div className="mb-8 rounded-2xl border border-line bg-white p-4">
          ...
          <Link href={`${base}/fundo`} className="mt-3 block text-center text-sm font-semibold text-blue">
            Ver Fundo da Equipa →
          </Link>
        </div>
        ...
        <ScrollReveal>
          <section className="mb-8 rounded-2xl bg-blue p-5 text-white">
            ...
            <p className="mt-2 text-sm text-white/80">
              Os miúdos têm o campeonato deles. Nós temos o nosso. Vamos escolher, prever,
              acertar, falhar, ganhar pontos e tentar chegar ao topo. No final, haverá um
              campeão. Mas o verdadeiro objetivo é outro: que todos contribuamos para tornar
              esta época ainda melhor para os nossos miúdos.
            </p>
        ...
        <div className="mb-4 grid grid-cols-3 gap-2 text-center text-xs font-semibold text-blue">
          <Link href={`${base}/plantel`} className="rounded-xl border border-line bg-white py-3">
            Plantel
          </Link>
          <Link href={`${base}/fundo`} className="rounded-xl border border-line bg-white py-3">
            Fundo
          </Link>
          {hasTeam && parent.isAdmin && (
            <Link href={`${base}/admin`} className="rounded-xl border border-gold bg-gold/10 py-3 text-ink">
              Admin
            </Link>
          )}
        </div>
